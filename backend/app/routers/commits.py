...
router = APIRouter(
    prefix="/commit/{user_name}/{repository_name}/{branch_name}", tags=["commit"]
)
...
def commit_files(
    ...
    try:
        ...
        repository = crud.get_one_or_error(
            db, models.Repository, name=repository_name, creator_id=user.id
        )
        branch = crud.get_one_or_error(
            db, models.Branch, name=branch_name, repository_id=repository.id
        )
        # Put files into db
        new_objects = []
        for file in files:
            ...
            object = crud.create_or_get(
                db, models.Object, name=file.filename, blob=contents, oid=str(obj_oid)
            )
        ...
        is_commit_in_db = crud.get_one_or_none(
            db, models.CommitObjectAssociation, commit_oid=commit_oid
        )
...
@router.get("/{commit_id}/objects", status_code=status.HTTP_200_OK)
def get_all_objects_for_commit(
    commit_id: int,
    repository_name: str,
    user_name: str,
    db: Session = Depends(database.get_db),
    ...
    commit = crud.get_one_or_error(
        db, models.Commit, id=commit_id, repository_id=repository.id
    )
    objects = commit.objects
    return {"commit": commit, "objects": objects}


@router.get(
    "/all",
    response_model=List[schemas.CommitOverviewResponseSchema],
